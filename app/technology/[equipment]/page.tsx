...
export default function Crew({ params }: { params: { equipment: string } }) {
  ...
  return (
    <div className="flex size-full flex-col-reverse items-center justify-between gap-[32px] pb-24 xl:max-h-[734px] xl:flex-row xl:gap-0 xl:pb-0">
      <div className="flex size-full max-h-[304px] flex-col items-center justify-between gap-[40px] xl:max-w-[635px] xl:flex-row xl:gap-0">
            ...
            <li className="group/li flex flex-col justify-between" key={item[0]}>
              <Link prefetch href={item[1]}>
                <button
                  ...
                  type="button"
                  className={`size-[56px] rounded-full border font-bellefair text-[24px] transition xl:size-[80px] xl:text-[32px] ${option === item[0] ? 'border-white bg-white text-veryDarkNavy' : 'border-white/25 bg-transparent text-white hover:border-white'}`}
                >
                  {index + 1}
                </button>
              </Link>
        ...
        <div className="flex size-full max-h-[301px] max-w-[491px] flex-col items-center text-center xl:items-start xl:text-start">
          ...
          <p className="mt-[24px] font-barlow text-[16px] leading-[180%] text-lightBlue xl:text-[18px]">
      ...
      <div className="relative flex size-full items-center justify-center pt-16 text-white xl:max-w-[608px] xl:pl-4 xl:pt-0">
        <>
          ...
          />
          <Image
            className={`relative -z-10 block h-[357px] w-fit object-cover xl:hidden xl:h-[322px] ${option === 'spaceport' ? 'object-[100%_10%]' : ''}`}
            width={608}
            height={600}
